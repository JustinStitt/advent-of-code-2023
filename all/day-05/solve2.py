...
# Advent of Code 2023 - Day 5 pt 2
import sys
import itertools
import functools
import re
...
class Soln:
    ...
    def solve(self):
        ...
        range_objects = [
            range(int(x), int(y) + int(x)) for (x, y) in zip(seeds[::2], seeds[1::2])
        ]
        ic(range_objects)
        # fmt: off
        for m in [seed_to_soil, soil_to_fert, fert_to_water, water_to_light, light_to_temp, temp_to_humid, humid_to_location]:
            ...
            for ro in range_objects:
                for k, v in m.items():
                    dr = v(k.start)
                    if k.start in ro and k.stop in ro: # k entirely in ro
                        nro.append(range(dr, dr+len(k)))
                    elif k.start in ro and k.stop not in ro: # left tail in
                        nro.append(range(dr, ro.stop+dr))
                    elif k.start not in ro and k.stop in ro: # right tail in
                        nro.append(range(dr+ro.start, dr+len(k)))
                    else: # not in at all
                        ...
